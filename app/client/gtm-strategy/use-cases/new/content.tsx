...
export default function NewUseCaseContent() {
  ...
  useEffect(() => {
    const hasContent = 
      ...
      formData.scenarios.some(item => item.trim() !== '') ||
      formData.desiredOutcomes.some(item => item.trim() !== '') ||
      formData.businessDrivers.some(item => item.trim() !== '') ||
      formData.businessImpact.some(item => item.trim() !== '');
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      if (!formData.name.trim()) { toast.error('Use case name is required'); return; }

      const { data: { session } } = await supabase.auth.getSession();
      const authToken = session?.access_token;
      if (!authToken) throw new Error('Authentication required');

      const url = impersonateUserId ? `/api/client/gtm-strategy/use-cases?impersonate=${impersonateUserId}` : '/api/client/gtm-strategy/use-cases';
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold text-fo-dark">Create New Use Case</h1>
            </div>
          </div>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-fo-primary text-white rounded-lg hover:bg-fo-primary/90 font-semibold transition-colors disabled:opacity-50">
            {saving ? (<><Loader2 className="w-4 h-4 animate-spin" strokeWidth={2} />Creating...</>) : (<><Save className="w-4 h-4" strokeWidth={2} />Create Use Case</>)}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-fo-border p-6 space-y-6">
        ...
        {renderArrayField('Scenarios', 'scenarios', 'Scenario')}
        {renderArrayField('Desired Outcomes', 'desiredOutcomes', 'Desired outcome')}
        {renderArrayField('Business Drivers', 'businessDrivers', 'Business driver')}
        {renderArrayField('Business Impact', 'businessImpact', 'Business impact')}
      </div>
