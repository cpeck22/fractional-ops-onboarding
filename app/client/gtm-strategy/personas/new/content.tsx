...
export default function NewPersonaContent() {
  ...
  useEffect(() => {
    const hasContent = 
      ...
      formData.primaryResponsibilities.some(r => r.trim() !== '') ||
      formData.painPoints.some(p => p.trim() !== '') ||
      formData.keyConcerns.some(k => k.trim() !== '') ||
      formData.keyObjectives.some(o => o.trim() !== '') ||
      formData.commonJobTitles.some(j => j.trim() !== '') ||
      formData.whyTheyMatterToUs.some(w => w.trim() !== '') ||
      formData.whyWeMatterToThem.some(w => w.trim() !== '');
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      if (!formData.name.trim()) { toast.error('Persona name is required'); return; }

      const { data: { session } } = await supabase.auth.getSession();
      const authToken = session?.access_token;
      if (!authToken) throw new Error('Authentication required');

      const url = impersonateUserId ? `/api/client/gtm-strategy/personas?impersonate=${impersonateUserId}` : '/api/client/gtm-strategy/personas';
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold text-fo-dark">Create New Persona</h1>
            </div>
          </div>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-fo-primary text-white rounded-lg hover:bg-fo-primary/90 font-semibold transition-colors disabled:opacity-50">
            {saving ? (<><Loader2 className="w-4 h-4 animate-spin" strokeWidth={2} />Creating...</>) : (<><Save className="w-4 h-4" strokeWidth={2} />Create Persona</>)}
          </button>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-2 border border-fo-border rounded-lg focus:outline-none focus:ring-2 focus:ring-fo-primary" placeholder="Persona name (e.g., VP of Sales)" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-4 py-2 border border-fo-border rounded-lg focus:outline-none focus:ring-2 focus:ring-fo-primary" placeholder="Brief description of the persona" />
            </div>
          </div>
        </div>

        {renderArrayField('Common Job Titles', 'commonJobTitles', 'Job title (e.g., VP of Sales)')}
        {renderArrayField('Primary Responsibilities', 'primaryResponsibilities', 'Responsibility')}
        {renderArrayField('Pain Points', 'painPoints', 'Pain point')}
        {renderArrayField('Key Concerns', 'keyConcerns', 'Concern')}
        {renderArrayField('Key Objectives', 'keyObjectives', 'Objective')}
        {renderArrayField('Why They Matter to Us', 'whyTheyMatterToUs', 'Reason they matter')}
        {renderArrayField('Why We Matter to Them', 'whyWeMatterToThem', 'Reason we matter')}
      </div>
