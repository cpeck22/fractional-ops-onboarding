...
import { Layers, Loader2, ArrowLeft, Save } from 'lucide-react';
import toast from 'react-hot-toast';
import { supabase } from '@/lib/supabase';
import ConfirmDeleteModal from '@/components/ConfirmDeleteModal';
import UnsavedChangesWarning from '@/components/UnsavedChangesWarning';

export default function NewSegmentContent() {
  ...
  useEffect(() => {
    const hasContent = 
      ...
      formData.fitExplanation.trim() !== '' ||
      formData.keyPriorities.some(item => item.trim() !== '') ||
      formData.keyConsiderations.some(item => item.trim() !== '') ||
      formData.uniqueApproach.some(item => item.trim() !== '');
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold text-fo-dark">Create New Segment</h1>
            </div>
          </div>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-fo-primary text-white rounded-lg hover:bg-fo-primary/90 font-semibold transition-colors disabled:opacity-50">
            {saving ? (<><Loader2 className="w-4 h-4 animate-spin" strokeWidth={2} />Creating...</>) : (<><Save className="w-4 h-4" strokeWidth={2} />Create Segment</>)}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-fo-border p-6 space-y-6">
        ...
        {renderArrayField('Key Priorities', 'keyPriorities', 'Priority')}
        {renderArrayField('Key Considerations', 'keyConsiderations', 'Consideration')}
        {renderArrayField('Unique Approach', 'uniqueApproach', 'Approach element')}
      </div>
