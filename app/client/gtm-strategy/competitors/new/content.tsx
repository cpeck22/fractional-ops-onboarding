...
export default function NewCompetitorContent() {
  ...
  useEffect(() => {
    const hasContent = 
      ...
      formData.businessModel.some(item => item.trim() !== '') ||
      formData.comparativeStrengths.some(item => item.trim() !== '') ||
      formData.comparativeWeaknesses.some(item => item.trim() !== '') ||
      formData.keyDifferentiators.some(item => item.trim() !== '') ||
      formData.reasonsWeWin.some(item => item.trim() !== '');
