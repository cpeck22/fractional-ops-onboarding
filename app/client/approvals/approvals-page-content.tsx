...
export default function ApprovalsPageContent() {
  ...
  const handleDeleteDraft = async (executionId: string, playName: string) => {
    ...
    // Store the execution and its index before removing it (in case we need to restore it)
    const executionIndex = executions.findIndex(e => e.id === executionId);
    const executionToDelete = executionIndex >= 0 ? executions[executionIndex] : null;
    
    // Optimistically remove from UI immediately
    setExecutions(prev => prev.filter(e => e.id !== executionId));
    
    // Update stats optimistically
    if (executionToDelete?.status === 'draft') {
      setStats(prev => ({
        ...prev,
    ...
    try {
      ...
      if (result.success) {
        ...
        // Don't reload - we already optimistically removed it
        // Reloading would bring back cached/stale data
      } else {
        // If deletion failed, restore the execution
        if (executionToDelete && executionIndex >= 0) {
          setExecutions(prev => {
            // Insert it back in the same position
            const newExecutions = [...prev];
            newExecutions.splice(executionIndex, 0, executionToDelete);
