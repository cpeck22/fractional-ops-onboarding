-- ============================================
-- ADD PLAY PROMPTS AND AGENT ID COLUMNS
-- ============================================
-- Adds columns to claire_plays table to store:
-- - agent_id: The Octave agent ID for this play
-- - prompt_template: The CEO's prompt template with variables
-- ============================================

-- Add agent_id column
ALTER TABLE claire_plays 
ADD COLUMN IF NOT EXISTS agent_id TEXT;

-- Add prompt_template column (stores the full CEO prompt)
ALTER TABLE claire_plays 
ADD COLUMN IF NOT EXISTS prompt_template TEXT;

-- Add index for agent_id lookups
CREATE INDEX IF NOT EXISTS idx_claire_plays_agent_id ON claire_plays(agent_id) WHERE agent_id IS NOT NULL;

-- Update existing plays with agent IDs and prompts from play-prompts.ts
-- Play 0002
UPDATE claire_plays 
SET 
  agent_id = 'ca_Q2MtCQAuQCmHilPWUYpyr',
...
UPDATE claire_plays 
SET 
  agent_id = 'ca_c1qy7EuAXr8Z6TPujnycr',
  prompt_template = 'LinkedIn 1A - Day 1 - LinkedIn Connection Request
